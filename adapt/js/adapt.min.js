...
define('core/js/models/questionModel',[
    ...
    var QuestionModel = ComponentModel.extend({
        ...
        getFeedbackTitle: function() {
            if (this.get('_isCorrect') == false) {
                return this.get('_feedback')._incorrect.title;
            } else {
...
define('core/js/scrolling',[
  ...
  Adapt.scrollTo = function(selector, settings) {
    ...
    _.delay(function() {
      Adapt.a11y.findFirstReadable(selector);      
      $(selector).find(".title_body span").removeAttr("tabindex");
      console.log(selector,"This is from SCROLL.js") 
      Adapt.trigger(location+':scrolledTo', selector);
      setTimeout(function(){
        $(".title_body span").attr("tabindex","0");
        $(selector).find(".title_body span").focus()
		},50)
    }, settings.duration+450);
  };
...
define('core/js/views/buttonsView',[
    ...
    var ButtonsView = Backbone.View.extend({
        ...
        onReset: function(e) {
            this.trigger('buttons:stateUpdate', BUTTON_STATE.RESET);
            $(e.target).addClass("is_disabled_btn");

            $(e.target).closest(".component__container").find(".btn-submit").removeClass("is_disabled_btn");
        },
...
define('core/js/views/questionView',[
  ...
  var QuestionView = ComponentView.extend({
    ...
    onSubmitClicked: function() {
      ...
      // if(this.model.get("_isCorrect") || $(this.el).hasClass("is-reseted")){
      //   this.showFeedback();
      // }
      ...
      var attmt=this.model.get("_attemptsLeft");      
      if(this.model.get("_isCorrect") ){        
        this.$el.closest(".component__container").find(".btn-submit").addClass("is_disabled_btn");
        this.$el.closest(".component__container").find(".btn-submit").text("Submit");
        //this.$el.closest(".component__container").find(".btn-reset").removeClass("is_disabled_btn");
        this.$el.closest(".component__container").find(".btn-submit").attr("aria-label","Submit")
        //this.$el.closest(".component__container").find(".btn-reset").attr("aria-label","Reset")  
        //this.$el.closest(".component__container").find(".btn-reset").text("Retry");
      }
      else if(this.model.get("_isCorrect")==undefined || attmt===1){
        this.$el.closest(".component__container").find(".btn-submit").addClass("is_disabled_btn");
        this.$el.closest(".component__container").find(".btn-submit").text("Submit");
        this.$el.closest(".component__container").find(".btn-reset").removeClass("is_disabled_btn");
        this.$el.closest(".component__container").find(".btn-submit").attr("aria-label","Submit")
      ...
      if(this.model.get("_isCorrect") || (attmt===0)){        
       var __options = this.model.get("_items");
         var that = this;
         
         $.each(__options, function(k,v){   

             if(v._shouldBeSelected){
                $(that.el).find(".item-"+k).addClass("show_correctmark_icons")
             }
             else{
               $(that.el).find(".item-"+k).addClass("show_incorrectmark_icons")
             }
             $(that.el).find(".item-"+k).find('.mcq__item-label').attr("tabindex", -1);
         })         
         $(that.el).find(".mcq__item").addClass("is-disabled-option");                
      }    
    ...
    onResetClicked: function() {                 
      if($(this.el).find(".component__widget").hasClass("is-submitted")){
        $(this.el).addClass("is-reseted");
        $(this.el).find("input:checked").siblings("label").addClass("is-option-selected")
        $(this.el).find(".mcq__item").removeClass("is-incorrect_mark is-correct_mark");             
      }

      
     
      this.setQuestionAsReset();     
      $(this.el).closest(".component__container").find(".feedback-container").slideUp().remove();      
      this._runModelCompatibleFunction("updateButtons");
...
define('components/adapt-contrib-mcq/js/mcqView',[
    ...
    var McqView = QuestionView.extend({
        ...
        events: {
            ...
            'click .js-item-label': 'onlabelClick',
            'click .mcq__item-label': 'mmcqChecboxClick',
            'click .mmcq-feedback-btn': 'mmcqfeedbackClickBtn',
            'keypress .js-item-label': 'onlabelkeypress',
            'focus .mcq__body-inner': "resetAnsOnfocus"
        },
        ...
        mmcqChecboxClick: function() {


            $(".MMCQ_feedback .btn__container").addClass("mmcq-feedback-btn");

        },
        mmcqfeedbackClickBtn: function() {

            $(".mcq__item-label-feedback").css("display", "block");
        ...
        onlabelClick: function(e) {
            ...
            if ($(e.currentTarget).is(".is-disabled") && !$(e.currentTarget).is(".is-selected") && $('.is-partially-correct').is(":visible")) {
                this.onResetClicked();
        ...
        // resetAnsOnfocus: function(e){

        // if($(e.currentTarget).closest(".component__inner").find(".mcq__item-label").is('.is-disabled') && $(e.currentTarget).closest(".component__inner").find(".mcq__item-label").is('.is-selected') && $('.is-partially-correct').is(":visible"))  {
        //     this.onResetClicked();
        //     setTimeout(function(){
        //       $(e.currentTarget).click()
        //     },10)        
        ...
        onItemSelect: function(event) {
            this.$(".btn-submit").removeClass("is_disabled_btn");
            if (!this.model.isInteractive()) return;
        ...
        updateSelection: function() {
            ...
            var attemp = this.model.get("_attemptsLeft");
            this.model.getChildren().each(function(itemModel) {
                ...
                if (this.model.get("_isCorrect") === false && attemp === 1) {
                    if (isSelected == true) {
        ...
        onSubmitted: function() {
            ...
            this.$('input:checked').each(function(k, v) {
                var inx = $(v).data('adapt-index')
                var opt = that.model.get("_items")[+inx];
                that.$('.item-' + inx).addClass(opt._shouldBeSelected ? 'is-correct_mark' : 'is-incorrect_mark');
            });
...
define('components/adapt-contrib-timeline/js/adapt-contrib-timeline',[
  'core/js/adapt',
  'core/js/views/componentView',
  'core/js/models/componentModel'
], function(Adapt, ComponentView, ComponentModel) {
  ...
  var TimelineView = ComponentView.extend({
    ...
    postRender: function() {
      ...
      this.setupInview();
      $(this.el).closest('.block').css('opacity','1');
    },
...
define('extensions/adapt-contrib-trickle/js/trickleView',[
  ...
  var TrickleView = Backbone.View.extend({
    ...
    onPostRender: function(view) {      
      console.log(view.model.get("_id"));
...
define('extensions/adapt-contrib-trickle/js/handlers/buttonView',[
  ...
  var TrickleButtonView = Backbone.View.extend({
    ...
    events: {
      ...
      "click .js-prev-btn": "onPrevClick"

    },

    onPrevClick:function(e){
      var id = this.model.get('_id');
      var blocks = Adapt.blocks.models.map(function(v){return v.attributes._id});
      var inx = blocks.indexOf(id)<=0?0:blocks.indexOf(id)-1;
      $('html, body').animate({
        scrollTop: $('[data-adapt-id="'+blocks[inx]+'"]').offset().top
    }, 500);
    },
    ...
    setButtonEnabled: function(isEnabled) {
      ...
      if (isEnabled) {
        if(!this.model.get('_trickle')._isFinal)
    ...
    Adapt.on('questionView:showFeedback', function(view) {
        ...
        if (view.model.has('_isCorrect')) {
            ...
            if (view.model.get('_isCorrect')) {
                ...
                alertObject._isCorrect = true;

            } else {
                if (view.model.has('_isAtLeastOneCorrectSelection')) {
                    // Partially correct feedback is an option.

                    if (view.model.get('_isAtLeastOneCorrectSelection') || view.model.attributes._attemptsLeft === 1) {
                        classes.push('is-partially-correct');
                        var body = view.model.get('_feedback')._partlyCorrectatt1 ? view.model.get('_feedback')._partlyCorrectatt1.final1 + "<br>" + view.model.get("feedbackMessage") : view.model.get("feedbackMessage");
                        alertObject = {
                            title: view.model.get('feedbackTitle'),
                            body: body,
                            _isCorrect: false
                        ...
                        var body = view.model.get('_feedback')._partlyCorrect ? view.model.get("feedbackMessage") + "<br>" + view.model.get('_feedback')._partlyCorrect.final : view.model.get("feedbackMessage");
                        alertObject = {
                            title: view.model.get('feedbackTitle'),
                            body: body,
                            _isCorrect: false
        ...
        alertObject._accessibility = Adapt.course.get('_globals')._accessibility;
        Adapt.once('notify:closed', function() {
            Adapt.trigger('tutor:closed', view, alertObject);
        });

        var feeback = Handlebars.templates['questionFeedback'];
        $(view.$el).closest('.component__container').append(feeback(alertObject));
        $(view.$el).closest('.component__container').find('.feedback-container').slideDown(500);
        setTimeout(function() {
            $(view.$el).closest('.component__container').find('#notify-heading').focus();
        }, 500);
